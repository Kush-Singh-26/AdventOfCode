...
bool check_valid(uint64_t ans , vector<uint64_t> nums)
{
    ...
    for(int i =0;i<nums.size();i++)
    {
        ...
        for(int j = 0; j <curr_size;j++)
        {
            ...
            uint64_t concatenated = currval * pow(10, int(log10(nums[i])) + 1) + nums[i];

            if (added <= ans) possible_values.push(added);
            if (multiplied <= ans) possible_values.push(multiplied);
            if(concatenated <= ans) possible_values.push(concatenated);
        }
    }
    
    /*
        TO concatenate 2 numbers num1 and num2 :
        nums3 = num1 * pow(10, noofdigits(num2))) + nun2
        no of digits in num2 = (log(num2)) + 1
    */
